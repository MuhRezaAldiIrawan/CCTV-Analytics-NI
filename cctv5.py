    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if int(output[1]+(output[3]-output[1])/2) > (240) and int(output[1]+(output[3]-output[1])/2) < (300) and int(output[0]+(output[2]-output[0])/2) < (390) and int(output[0]+(output[2]-output[0])/2) > (270):
                            annotator.box_label(
            ...
            if show_vid:
                ...
                start_point = (270, 245)
                end_point = (390, 250)
                start_point1 = (190, 290)
                end_point1 = (380, 305)
                start_point2 = (270, 245)
                end_point2 = (190, 290)
                start_point3 = (390, 250)
                end_point3 = (380, 305)
                ...
                cv2.putText(im0, 'Mobil = ' + str(car1), org1, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                cv2.putText(im0, 'Truk and Bus = ' + str(truck1), org2, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                cv2.putText(im0, 'Pelanggaran = ' + str(person), org3, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                # cv2.putText(im0,'fps= ' + str(fps), org3, font,
                # fontScale, color, thickness, cv2.LINE_AA)
...
def count_obj1(box, w, h, id):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (240) and int(box[1]+(box[3]-box[1])/2) < (310):
        if id not in data:
            count1 = truck1 + car1
            data.append(id)
...
def count_car(box, w, h, id, im0):
    ...
    center_coordinates = (
        int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if int(box[1]+(box[3]-box[1])/2) > (240) and int(box[1]+(box[3]-box[1])/2) < (300) and int(box[0]+(box[2]-box[0])/2) < (390) and int(box[0]+(box[2]-box[0])/2) > (270):
...
def count_pelanggaran(box, w, h, id, dir2, im0, gambar, simpan):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (240) and int(box[1]+(box[3]-box[1])/2) < (300) and int(box[0]+(box[2]-box[0])/2) < (390) and int(box[0]+(box[2]-box[0])/2) > (270):
        if id not in data2:
            ...
            # body = """
            # Pelanggaran di On Ramp Alauddin 1"""
            # # SMPTP
            # s = smtplib.SMTP('smtp.gmail.com', 587)
            # # start TLS for security
            # s.starttls() 
...
def inputdata(car1, truck1, count1):
    ...
    truck = str(truck1)
    count = str(count1)
    lokasi = 'On Ramp Alauddin'
    # membuat query untuk insert data ke mysql
    # sql = "INSERT INTO on_ramp_pettarani(date, Mobil, Bus_Truk, total) VALUES (now(), '" + \
    #     car+"', '"+truck+"', '"+count+"')"
