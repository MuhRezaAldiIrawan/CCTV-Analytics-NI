...
receiver_id = [6056643678,5431566487, 320851322, 664202412, 128169865] # https://api.telegram.org/bot<TOKEN>/getUpdates
...
def current(opt):
    ...
    out, source6, yolo_model2, deep_sort_model, show_vid, save_vid, save_txt, imgsz, evaluate, half, project, name, exist_ok = \
        opt.output, opt.source6, opt.yolo_model2, opt.deep_sort_model, opt.show_vid, opt.save_vid, \
        opt.save_txt, opt.imgsz, opt.evaluate, opt.half, opt.project, opt.name, opt.exist_ok
    webcam = source6 == '0' or source6.startswith(
        'rtsp') or source6.startswith('http') or source6.endswith('.txt')
    ...
    if webcam:
        ...
        dataset = LoadStreams(source6, img_size=imgsz,
                              stride=stride, auto=pt and not jit)
        bs = len(dataset)  # batch_size
    else:
        dataset = LoadImages(source6, img_size=imgsz,
                             stride=stride, auto=pt and not jit)
        bs = 1  # batch_size
    ...
    txt_file_name = source6.split('/')[-1].split('.')[0]
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        label = f'{id}'
                        if names[c] == 'car' or names[c] == 'bus' or names[c] == 'truck':
                            global masuk, keluar, t7
                            dir3 = save_dir / 'melawan_arus' / f'{id}.jpg'
                            l = str(Path(save_dir))
                            simpan=l.lstrip('static')
                            detect_current(bboxes, w, h, id, im0, simpan, dir3)
                        ...
                        # if int(output[1]+(output[3]-output[1])/2) > (225) and int(output[1]+(output[3]-output[1])/2) < (385) and int(output[0]+(output[2]-output[0])/2) < (410) and int(output[0]+(output[2]-output[0])/2) > (270):
                        annotator.box_label(
            ...
            if show_vid:
                global count1, car1, truck1, loop_time
                color = (200, 255, 100)
                color1 = (0, 255, 255)
                start_point = (130, 160)
                end_point = (290, 200)
                ...
                fontScale = 1
                cv2.putText(im0, 'Melawan Arus = ' + str(masuk), org, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                cv2.putText(im0, 'Pelanggaran Lainnya = ' + str(person), org1, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                cv2.putText(im0, 'OUT = ' + str(keluar), org2, font,
                            fontScale, color, thickness, cv2.LINE_AA)
...
def detect_current(box, w, h, id, im0, simpan, dir3):
    global masuk, data1, count1, keluar, t7, t6
    gambar = f'{id}.jpg'
    center_coordinates = (
    ...
    if int(box[1]+(box[3]-box[1])/2) > (160) and int(box[1]+(box[3]-box[1])/2) < (200) and int(box[0]+(box[2]-box[0])/2) < (150) :
        if id not in data1:
            masuk += 1
            data1.append(id)
            print('Melawan Arus : '+str(masuk))
            save_one_box(box, im0, file=dir3, BGR=True)
            try :
                
                sql = "INSERT INTO data_pelanggaran(JENIS_PELANGGARAN, WAKTU, GAMBAR, LOKASI, project) VALUES ( 'Melawan Arus', now(), '"+gambar+"', 'Off Ramp Rappokalling', '"+str(simpan)+ "')"
                print(sql)
                mysqlCursor.execute(sql)
                mysql.commit()
                
                for x in receiver_id:
                    bot.sendPhoto(x, photo=open(dir3, 'rb')) # send message to telegram
                    bot.sendMessage(x, 'Ada Pelanggaran di Off Ramp Rappokalling, Melawan Arus')
                
            except :
                pass


    if int(box[1]+(box[3]-box[1])/2) > (200) and int(box[1]+(box[3]-box[1])/2) < (300) and int(box[0]+(box[2]-box[0])/2) < (150) :
        if id not in data1:
            keluar += 1
            data1.append(id)
...
def inputdata(jumlah):
    ...
    # sql = "INSERT INTO off_ramp_rappokalling(date, jumlah) VALUES (now(), '"+jumlah+"')"
    sql_counting = "INSERT INTO table_counting(date,  total, lokasi) VALUES (now(),  + \
         '"+jumlah+"', '"+lokasi+"')"
    ...
    # mysqlCursor.execute(sql)
    mysqlCursor.execute(sql_counting)
