        ...
        for i, det in enumerate(pred):  # detections per image
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if names[c] == 'person' or names[c] == 'motorcycle' or names[c] == 'bicycle':
                            ...
                            # count_person(bboxes, w, h, id, dir2, im0,gambar, simpan)
            ...
            if show_vid:
                ...
                cv2.putText(im0, 'Pelanggaran = ' + str(person), org3, font,
                            fontScale, color, thickness, cv2.LINE_AA)
                # cv2.putText(im0,'fps= ' + str(fps), org4, font,
                # fontScale, color, thickness, cv2.LINE_AA)
...
def count_obj(box, w, h, id):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (480) and int(box[1]+(box[3]-box[1])/2) < (580):
        if id not in data:
            count = truck + car
            data.append(id)
...
def count_person(box, w, h, id, dir2, im0, gambar, simpan):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (0):
        if id not in data2:
            ...
            for x in receiver_id:
                ...
                bot.sendMessage(x, 'Pelanggaran, Ada Orang di Gerbang Toll Kaluku Badoa') 
