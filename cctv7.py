...
def gen3(opt):
    ...
    out, source7, yolo_model2, deep_sort_model, show_vid, save_vid, save_txt, imgsz, evaluate, half, project, name, exist_ok = \
        opt.output, opt.source7, opt.yolo_model2, opt.deep_sort_model, opt.show_vid, opt.save_vid, \
        opt.save_txt, opt.imgsz, opt.evaluate, opt.half, opt.project, opt.name, opt.exist_ok
    webcam = source7 == '0' or source7.startswith(
        'rtsp') or source7.startswith('http') or source7.endswith('.txt')
    ...
    if webcam:
        ...
        dataset = LoadStreams(source7, img_size=imgsz,
                              stride=stride, auto=pt and not jit)
        bs = len(dataset)  # batch_size
    else:
        dataset = LoadImages(source7, img_size=imgsz,
                             stride=stride, auto=pt and not jit)
        bs = 1  # batch_size
    ...
    txt_file_name = source7.split('/')[-1].split('.')[0]
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if int(output[1]+(output[3]-output[1])/2) > (125) and int(output[1]+(output[3]-output[1])/2) < (205) and int(output[0]+(output[2]-output[0])/2) < (150) and int(output[0]+(output[2]-output[0])/2) > (50):
...
def count_obj1(box, w, h, id):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (125) and int(box[1]+(box[3]-box[1])/2) < (200):
        if id not in data:
            count1 = truck1 + car1
            data.append(id)
...
def count_car(box, w, h, id, im0):
    ...
    center_coordinates = (
        int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if int(box[1]+(box[3]-box[1])/2) > (125) and int(box[1]+(box[3]-box[1])/2) < (205) and int(box[0]+(box[2]-box[0])/2) < (150) and int(box[0]+(box[2]-box[0])/2) > (50):
        cv2.putText(im0, 'Cordinate = ' + str(int(box[0]+(box[2]-box[0])/2))+','+str(int(box[1]+(box[3]-box[1])/2)), (20,80), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, (50, 255 ,125), 2, cv2.LINE_AA)
...
#     center_coordinates = (
#         int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
#     if int(box[1]+(box[3]-box[1])/2) > (140) and int(box[1]+(box[3]-box[1])/2) < (330) and int(box[0]+(box[2]-box[0])/2) < (150) and int(box[0]+(box[2]-box[0])/2) > (50):
#         if id not in data2:
#             person += 1
#             data2.append(id)
#             jp = 'Ada Orang'
...
def inputdata(car1, truck1, count1):
    ...
    truck = str(truck1)
    count = str(count1)
    lokasi = 'Off Ramp Tallo Timur'
    # membuat query untuk insert data ke mysql
    # sql = "INSERT INTO off_ramp_tallo_timur(date, Mobil, Bus_Truk, total) VALUES (now(), '" + \
        # car+"', '"+truck+"', '"+count+"')"
    sql_counting = "INSERT INTO table_counting(date, Mobil, Bus_Truk, total, lokasi) VALUES (now(), '" + \
        car+"', '"+truck+"', '"+count+"', '"+lokasi+"')"
